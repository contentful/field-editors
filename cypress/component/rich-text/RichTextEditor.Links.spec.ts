...
describe('Rich Text Editor - Links', { viewportHeight: 2000, viewportWidth: 1000 }, () => {
  ...
  for (const [triggerMethod, triggerLinkModal] of methods) {
    describe(triggerMethod, () => {
      ...
      it('converts text to entry hyperlink with semantic recommendations', () => {
        safelyType('Advanced AI content strategy{selectall}');
        triggerLinkModal();
        const form = richText.forms.hyperlink;

        form.linkText.should('have.value', 'Advanced AI content strategy');
        form.submit.should('be.disabled');
