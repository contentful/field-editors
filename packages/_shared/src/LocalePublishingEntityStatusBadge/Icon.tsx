import React from 'react';

import { generateIconComponent } from '@contentful/f36-icon';

// TODO: use from forma icons, once it's changed there (currently a custom one here: https://github.com/contentful/forma-36/blob/main/packages/components/navbar/src/icons/CaretIcon.tsx)
export const ArrowDownIcon = generateIconComponent({
  name: 'ArrowDownIcon',
  viewBox: '0 0 12 20',
  path: (
    <path
      d="M3.03076 8C2.20109 8 1.73228 8.95209 2.23814 9.60971L5.20727 13.4696C5.60757 13.99 6.39223 13.99 6.79252 13.4696L9.76166 9.60971C10.2675 8.95209 9.79871 8 8.96904 8L3.03076 8Z"
      fill="currentColor"
    />
  ),
});
