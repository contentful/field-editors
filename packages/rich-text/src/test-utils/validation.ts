...
import { getSchemaWithNodeType } from '@contentful/rich-text-types/dist/lib/schemas';

import { toContentfulError } from './toContentfulError';
...
function buildSchemaErrors(validateSchema, path, errors) {
  const schemaErrors = validateSchema.errors.map((error) => toContentfulError(error, path));
  const constraintError = schemaErrors.find((e) => e.name === 'in');
  if (constraintError) {
