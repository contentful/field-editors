...
// Inline re-implementation of the original moment-based logic,
// kept here solely to verify parity with the date-fns migration.
type DateFormatFn = (date: Date | string, short?: boolean) => string;

const formatDate: DateFormatFn = (date, short) => {
...
const formatTime: DateFormatFn = (date) => moment.utc(date).local().format('h:mm A');

const formatDateAndTime: DateFormatFn = (date, short) =>
  `${formatDate(date, short)} at ${formatTime(date)}`;
...
describe('formatDateAndTime (moment)', () => {
