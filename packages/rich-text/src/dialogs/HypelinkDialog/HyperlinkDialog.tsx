...
type HyperlinkDialogState = {
  text?: string;
  uri?: string;
  entityLinks: Record<typeof LINK_TYPES.ENTRY | typeof LINK_TYPES.ASSET, EntityLink | null>;
  type: LinkType;
};
...
function getInitialState(
  ...
  let linkType: LinkType | undefined = type;

  if (target && isEntityLink) {
    linkType = target.sys.linkType as LinkType;
    if (linkType === LINK_TYPES.ENTRY || linkType === LINK_TYPES.ASSET) {
...
export function HyperlinkDialog({
  ...
  const setTargetEntity = (type: LinkType, entity: SelectableEntity | null) => {
    ...
    setState((currentState) => ({
      ...
      entityLinks: {
        ...
        [type]: entity ? entityToLink(entity) : null,
      },
    }));
  };
  ...
  const getValue = (): HyperlinkValue => {
    ...
    if (type === LINK_TYPES.URI) {
      ...
      linkValue.target = state.entityLinks[type] ?? undefined;
    }
    return linkValue;
  };
  ...
  const renderEntitySelector = (entityType: LinkType) => (
  ...
  const renderEntityField = () => {
    ...
    const resetEntity = () => setTargetEntity(type, null);
